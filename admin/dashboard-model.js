...
export function summarizeDashboard({orders,stock,batches,movements,slots,paymentAttempts=[]}, now = new Date()) {
  ...
  const activity=[...orders.map(o=>({at:o.created_at,label:'Order #'+o.order_number+' placed',href:'#orders/detail/'+encodeURIComponent(o.id)})),...paymentAttempts.filter(p=>p.status==='paid').map(p=>({at:p.verified_at,label:'Verified payment received · ₱'+Number(p.amount).toFixed(2),href:'#orders'})),...movements.filter(m=>items.has(batchMap.get(m.batch_id)?.item_id)).map(m=>{const item=items.get(batchMap.get(m.batch_id).item_id);return {at:m.created_at,label:item.name+' · '+m.reason+' · '+Number(m.quantity_delta)+' '+item.unit,href:'#inventory/restock'};})].sort((a,b)=>Date.parse(b.at)-Date.parse(a.at)).slice(0,5);
  return {
